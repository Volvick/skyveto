<?php
// File: payment_settings.php (REVISED to include UPI ID)
// Location: /admin/
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $upi_id = trim($_POST['upi_id']); // Get UPI ID from form
    ...
    $stmt = $conn->prepare("UPDATE payment_settings SET 
        ...
        upi_id = ? 
        WHERE id = 1");
    // Add 's' for the new string parameter
    $stmt->bind_param("issss", $online_payment_enabled, $paytm_environment, $paytm_merchant_id, $paytm_merchant_key, $upi_id);
...
<div class="bg-white p-6 rounded-xl shadow-lg max-w-2xl mx-auto">
    ...
    <form method="POST" class="space-y-6">
        ...
        <!-- NEW: UPI ID Section -->
        <h3 class="text-lg font-semibold text-gray-700">UPI QR / Manual Payments</h3>
        <div>
            <label for="upi_id" class="block font-medium">Your UPI ID</label>
            <input type="text" id="upi_id" name="upi_id" value="<?php echo htmlspecialchars($settings['upi_id'] ?? ''); ?>" class="w-full p-2 border rounded-md mt-1" placeholder="e.g., yourname@upi">
            <p class="text-xs text-gray-500 mt-1">Users will be shown a QR code to pay to this ID. Leave blank to disable this option.</p>
        </div>

        <hr>

        <h3 class="text-lg font-semibold text-gray-700">Paytm Payment Gateway</h3>
        <div>
        ...
        <div>
            <label for="paytm_environment" class="block font-medium">Paytm Environment</label>
