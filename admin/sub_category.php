...
<div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold">Manage Sub Categories</h1>
...
<div id="category-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50 p-4">
    <div class="bg-white rounded-lg p-6 w-full max-w-md">
        <h2 id="modal-title" class="text-xl font-bold mb-4">Add New Sub Category</h2>
        <form id="category-form">
            <input type="hidden" name="action" id="form-action" value="add">
            <input type="hidden" name="category_id" id="category_id">
            <input type="hidden" name="current_image" id="current_image">
            <div class="mb-4">
                <label for="name" class="block mb-1">Sub Category Name</label>
                <input type="text" id="name" name="name" required class="w-full p-2 border rounded-md">
            </div>
            <div class="mb-4">
                <label for="image" class="block mb-1">Sub Category Image</label>
            ...
            <div class="mb-4">
                ...
                <select name="parent_id" id="parent_id" required class="w-full p-2 border rounded-md">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const openModalForAdd = () => {
        form.reset();
        document.getElementById('modal-title').innerText = 'Add New Sub Category';
    ...
    const fetchCategories = async () => {
        ...
        formData.append('fetch_type', 'sub'); // Fetch only sub categories
        try {
            ...
            if (result.status === 'success' && result.data.length > 0) {
                ...
                tbody.innerHTML = '<tr><td colspan="4" class="text-center p-4">No sub categories found.</td></tr>';
            }
        } catch (error) {
