<?php
// File: parent_category.php (NEW)
// Location: /admin/
include_once 'common/header.php'; 
?>

<div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold">Manage Parent Categories</h1>
...
<div class="bg-white p-6 rounded-xl shadow-lg">
    <table class="w-full">
        <thead><tr class="border-b"><th class="text-left p-3">Image</th><th class="text-left p-3">Name</th><th class="text-right p-3">Actions</th></tr></thead>
...
<div id="category-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50 p-4">
    <div class="bg-white rounded-lg p-6 w-full max-w-md">
        <h2 id="modal-title" class="text-xl font-bold mb-4">Add New Parent Category</h2>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const openModalForAdd = () => {
        form.reset();
        document.getElementById('modal-title').innerText = 'Add New Parent Category';
    ...
    const openModalForEdit = (data) => {
        form.reset();
        document.getElementById('modal-title').innerText = 'Edit Parent Category';
    ...
    const fetchCategories = async () => {
        tbody.innerHTML = '<tr><td colspan="3" class="text-center p-8"><div class="animate-spin rounded-full h-12 w-12 border-t-4 border-blue-500 mx-auto"></div></td></tr>';
        const formData = new FormData(); 
        formData.append('action', 'fetch');
        formData.append('fetch_type', 'parent'); // Fetch only parent categories
        try {
            ...
            if (result.status === 'success' && result.data.length > 0) {
                ...
                tbody.innerHTML = '<tr><td colspan="3" class="text-center p-4">No parent categories found.</td></tr>';
            }
        } catch (error) {
            tbody.innerHTML = '<tr><td colspan="3" class="text-center p-4 text-red-500">Error loading categories.</td></tr>';
        }
    };
    ...
    tbody.addEventListener('click', async (e) => {
        ...
        if (deleteBtn) {
            if (!confirm('Are you sure you want to delete this category? Deleting a parent category will also delete all its sub-categories.')) return;
